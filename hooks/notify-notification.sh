#!/usr/bin/env bash
# claude-code-notify — Notification event hook
...
try:
    ...
    msg = data.get('message') or data.get('msg') or data.get('text') or ''
...
if [ -z "$MSG" ]; then
    MSG="$CCN_MSG_NOTIFICATION_FALLBACK"
fi
...
if [ -n "$CCN_SOUND_NOTIFICATION" ] && [ -f "/System/Library/Sounds/${CCN_SOUND_NOTIFICATION}.aiff" ]; then
    SOUND_ARG=(-sound "$CCN_SOUND_NOTIFICATION")
else
    SOUND_ARG=()
fi
