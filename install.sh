...
if [[ "$USE_DEFAULTS" == true ]]; then
  ...
  printf "    3) warp       — Warp\n"
  echo ""
  printf "  Use detected (%s)? Press Enter to accept, or type 1-3: " "$DETECTED_TERM"
  read -r term_choice

  case "$term_choice" in
    1) CHOSEN_TERMINAL="terminal" ;;
    2) CHOSEN_TERMINAL="iterm2" ;;
    3) CHOSEN_TERMINAL="warp" ;;
    "") CHOSEN_TERMINAL="$DETECTED_TERM" ;;
    *)
...
CCN_SUBTITLE_STOP="Task Complete"
CCN_SUBTITLE_IDLE="Teammate Idle"
CCN_SUBTITLE_PERMISSION="Permission Request"
CONF
